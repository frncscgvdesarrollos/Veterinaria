...
export default function VistaTurnosPeluqueriaVeterinaria() {
    ...
    // Filtrar los turnos para hoy
    const turnosHoy = turnos.filter(turno => {
        return new Date(turno.fecha).toDateString() === new Date().toDateString() && !turno.finalizado && !turno.cancelado;
    });

    // Filtrar los turnos finalizados para hoy
    const turnosFinalizados = turnos.filter(turno => {
        return new Date(turno.fecha).toDateString() === new Date().toDateString() && turno.finalizado;
    });

    // Filtrar los turnos cancelados para hoy
    const turnosCancelados = turnos.filter(turno => {
        return new Date(turno.fecha).toDateString() === new Date().toDateString() && turno.cancelado;
    });
    ...
    return (
        ...
        <div className="w-full overflow-x-auto sm:rounded-lg mt-8">
            <h1 className="text-2xl font-bold my-4 text-left underline text-violet-800 sm:text-left p-4">Turnos finalizados:</h1>
            <table className="w-full divide-y divide-gray-200 sm:w-1/3 text-sm ">
                <thead className="bg-gray-200">
                    <tr>
                        <th className="px-3 py-2 text-left text-gray-600 uppercase">Nombre</th>
        ...
        <div className="w-full overflow-x-auto sm:rounded-lg mt-8">
            <h1 className="text-2xl font-bold my-4 text-left underline text-violet-800 sm:text-left p-4">Turnos cancelados:</h1>
            <table className="w-full divide-y divide-gray-200 sm:w-1/3 text-sm ">
                <thead className="bg-gray-200">
                    <tr>
                        <th className="px-3 py-2 text-left text-gray-600 uppercase">Nombre</th>
